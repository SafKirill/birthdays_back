...
from api.actions.jwtToken import _check_jwt_token
...
async def _get_user_by_email_for_auth(email: str, session: AsyncSession) -> User:
...
async def get_current_user_from_token(token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(get_db)):
    ...
    if user is None:
        raise credentials_exception
    jwt_token = await _check_jwt_token(user.id, token, db)
    if jwt_token:
        return user
    else:
        raise credentials_exception